...
Your task is to set up a monorepo using pnpm workspaces. Here's what you need to do:

- Start by navigating to the 01-monorepo-forest-start directory. Inside this directory, create a new folder and name it packages.
- Next, go inside the packages folder. Here, you need to initialize a new package. Let's name this package firelane-one.
- Inside the firelane-one package, add the `index.js` file.

Once you've completed these steps, your challenge is to ensure that you're able to execute the @forestpark/firelane-one package from the root of the monorepo using pnpm.
