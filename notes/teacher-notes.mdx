...
1. Start by creating a `packages` directory. Inside this directory, make a new package called `firelane-one`. Then, at your project's root, set up a pnpm workspace.

2. Inside the `firelane-one` package, add the `index.js` file that includes the `calculateLeaves` function.Then, create a script at the project's root that can execute a JavaScript file.
